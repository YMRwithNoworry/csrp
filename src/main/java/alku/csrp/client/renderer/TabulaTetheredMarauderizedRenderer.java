...
/** Direct Tabula renderer retaining the animated marauder tether beam. */
public final class TabulaTetheredMarauderizedRenderer<T extends TetheredMarauderizedEntity>
        extends TabulaMobRenderer<T> {
    private static final int TETHER_SEGMENTS = 24;
    private static final float TETHER_RADIUS = 0.045F;

    public TabulaTetheredMarauderizedRenderer(EntityRendererProvider.Context context, String id,
                                              float shadowRadius) {
    ...
    public void render(T parasite, float entityYaw, float partialTick, PoseStack poseStack,
        ...
        Vec3 origin = parasite.getPosition(partialTick);
        Vec3 start = parasite.getEyePosition(partialTick).subtract(origin);
        ...
        if (side.lengthSqr() < 0.0001D) side = forward.cross(new Vec3(1.0D, 0.0D, 0.0D));
        side = side.normalize();
        ...
        for (int segment = 1; segment <= TETHER_SEGMENTS; segment++) {
            ...
            emitSegment(pose, consumer, previous, current, side, up, radius, red, green, blue);
            previous = current;
        }
    }
    ...
    private static Vec3 tetherPoint(Vec3 start, Vec3 end, Vec3 side, Vec3 up, float age, float progress) {
        ...
        return start.lerp(end, progress)
                .add(side.scale(Mth.sin(age * 0.55F + progress * 24.0F) * 0.075D * taper))
                .add(up.scale(Mth.cos(age * 0.42F + progress * 17.0F) * 0.045D * taper));
    }

    private static void emitSegment(PoseStack.Pose pose, VertexConsumer consumer, Vec3 start, Vec3 end,

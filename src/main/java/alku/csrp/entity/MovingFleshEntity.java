...
public final class MovingFleshEntity extends CrudeParasiteEntity {
    ...
    public static MovingFleshEntity spawnFromCorpse(ServerLevel serverLevel, LivingEntity corpse) {
        MovingFleshEntity flesh = ModEntities.MOVINGFLESH.get()
                .create(serverLevel, EntitySpawnReason.CONVERSION);
        if (flesh == null) {
            return null;
        }
        flesh.snapTo(corpse.getX(), corpse.getY(), corpse.getZ(), corpse.getYRot(), corpse.getXRot());
    ...
    private void meltIntoMergedParasite() {
        ...
        merged.snapTo(getX(), getY(), getZ(), getYRot(), getXRot());
    ...
    private Mob createMergedParasite(ServerLevel serverLevel) {
        ...
        if (type == null
                || !(type.create(serverLevel, EntitySpawnReason.MOB_SUMMONED) instanceof Mob merged)) {
            return null;
        }
        return merged;
    }

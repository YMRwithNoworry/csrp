        ...
        controllers.add(new AnimationController<>(this, "movement_controller", 4, state -> {
            ...
            if (isShadowClone()) {
                if (isShadowHitFlashing()) {
                    return state.setAndContinue(getDeltaMovement().horizontalDistanceSqr() >= 0.0001 ? CLONE_SHAKING_WALK : CLONE_SHAKING_IDLE);
                }
                return state.setAndContinue(getDeltaMovement().horizontalDistanceSqr() >= 0.0001 ? CLONE_WALK : CLONE_IDLE);
            }
            if (isShadowHitFlashing()) {
                return state.setAndContinue(getDeltaMovement().horizontalDistanceSqr() >= 0.0001 ? SHAKING_WALK : SHAKING_IDLE);
            }
